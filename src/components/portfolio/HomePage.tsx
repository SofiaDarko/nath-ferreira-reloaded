...
function ProjectCard({ project, onClick, index, lang }: { project: Project; onClick: () => void; index: number; lang: string }) {
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-2xl overflow-hidden cursor-pointer bg-[#161616] min-h-[160px] min-w-[280px] max-w-[380px] transition-all duration-300 hover:ring-2 hover:ring-accent"
      onClick={onClick}
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col">
      <div className="px-15 pt-10 flex-shrink-0 relative z-10">
        ...
        <div className="w-full h-px bg-border mt-7" />
      </div>

      <div
        className="flex-1 overflow-x-auto overflow-y-hidden relative px-15 py-7 cursor-grab active:cursor-grabbing no-scrollbar"
        ref={trackRef}
      ...
      >
        ...
        <div className="absolute bottom-5 right-7 flex items-center gap-2 font-body text-[11px] text-muted-foreground tracking-widest uppercase opacity-50 pointer-events-none">
